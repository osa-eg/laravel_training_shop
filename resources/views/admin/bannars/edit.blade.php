...
@section('content')
    ...
    <div class="breadcrumb-header justify-content-between">
        <div class="my-auto">
            <div class="d-flex">
                <h4 class="content-title mb-0 my-auto">@lang('Bannars')</h4><span class="text-muted mt-1 tx-13 mr-2 mb-0">/ @lang('Edit a Bannar') </span>
            </div>
        </div>
    ...
    <!-- row opened -->
    <form action="{{ route('bannars.update', $item) }}" method="post" class="row row-sm" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        <div class="col-md-6 ">
                ...
                <div class="card-body pt-0">
                    <div class="">
                        <div class="form-group">
                            ...
                            <input type="text" name="title" class="form-control" id="title" value="{{ old('title', $item->title) }}" placeholder="@lang('Enter bannar title')">
                            @error('title')
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="sub_title" class="form-control" id="sub_title" value="{{ old('sub_title', $item->sub_title) }}" placeholder="@lang('Enter bannar Sub Title')">
                            @error('sub_title')
                                <small class="text-danger d-block">{{ $message }}</small>
                            @enderror
                        </div>
                        <div class="form-group">
                            ...
                            <input type="url" name="url" class="form-control" id="url" value="{{ old('url', $item->url) }}" placeholder="@lang('Enter bannar URL')">
                            @error('url')
                                <small class="text-danger d-block">{{ $message }}</small>
                            @enderror
                        </div>
